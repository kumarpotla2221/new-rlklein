...
const DEFAULT_AGENCY_NAME = 'R.L. Klein & Associates';
const DEFAULT_AGENCY_LOCATION = 'Rolling Hills Estates, California';
const DEFAULT_AGENCY_EXPERIENCE = 'Over 40 years of specialized healthcare staffing experience';
const DEFAULT_AGENCY_SUPPORT = 'Dedicated recruiter support, direct payroll coordination, and credentialing assistance throughout your assignment';
const DEFAULT_AGENCY_CERTIFICATIONS = 'Certified Small, Minority, and Local Small Business Enterprise (SBE / MBE / LSBE / NMSDC)';
...
export function JobDetailsModal({ job, onClose, readOnly = false }: JobDetailsModalProps) {
  ...
  const termSummary = job.assignmentTerm || (job.assignmentDuration && job.assignmentDuration !== 'Ongoing'
    ? `${job.assignmentDuration} assignment`
    : 'Ongoing assignment with the potential for extension');

  const agencyName = job.agencyName || DEFAULT_AGENCY_NAME;
  const agencyLocation = job.agencyLocation || DEFAULT_AGENCY_LOCATION;
  const agencyExperience = job.agencyExperience || DEFAULT_AGENCY_EXPERIENCE;
  const agencySupport = job.agencySupport || DEFAULT_AGENCY_SUPPORT;
  const agencyCertifications = job.agencyCertifications || DEFAULT_AGENCY_CERTIFICATIONS;
    ...
    <div className="job-modal-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) onClose(); }}>
      <div className="job-modal" role="dialog" aria-modal="true" aria-labelledby="job-modal-title">
        <div className="job-modal__header">
          <h2 id="job-modal-title">{readOnly ? 'Job Preview' : 'Job Details'}</h2>
        ...
        <div className="job-modal__body">
          {readOnly && (
            <div className="job-modal__preview-banner">
              This is a preview of how the posting will appear to candidates. It has not been saved or published yet.
            </div>
          )}
          ...
          <div className="job-modal__section">
            ...
            <div className="job-modal__grid">
              ...
              <div className="job-modal__field">
                <span className="job-modal__field-label">Shifts Per Week</span>
                <span className="job-modal__field-value">{job.shiftsPerWeek ?? '—'}</span>
              </div>
              <div className="job-modal__field">
              ...
              <div className="job-modal__field">
                <span className="job-modal__field-label">Shift Type</span>
                <span className="job-modal__field-value">{job.shiftPattern || '—'}</span>
              </div>
            </div>
          </div>
          ...
          <div className="job-modal__section">
            ...
            <div className="job-modal__grid">
              ...
              <div className="job-modal__field">
                <span className="job-modal__field-label">End Date</span>
                <span className="job-modal__field-value">{job.endDate ? formatDate(job.endDate) : 'Open / Ongoing'}</span>
              </div>
              <div className="job-modal__field">
              ...
              <div className="job-modal__field">
                <span className="job-modal__field-label">Contract Type</span>
                <span className="job-modal__field-value">{job.contractType || '—'}</span>
              </div>
              <div className="job-modal__field">
                <span className="job-modal__field-label">Extension Available</span>
                <span className="job-modal__field-value">{job.extensionAvailable ? 'Yes' : 'No'}</span>
              </div>
            </div>
            ...
            <div className="job-modal__description">
              ...
              {job.showCompensation && job.compensationMin && job.compensationMax && (
                <p className="job-modal__desc-line">
                  ...
                  {job.payFrequency ? ` · paid ${job.payFrequency.toLowerCase()}` : ''}
                </p>
              )}
              ...
              {job.requiredCredentials.length > 0 && (
                <>
                  <h4 className="job-modal__subheading">Required Credentials</h4>
                  <ul className="job-modal__bullet-list">
                    {job.requiredCredentials.map((cred, idx) => (
                      <li key={idx}><span className="job-modal__bullet-dot" />{cred}</li>
                    ))}
                  </ul>
                </>
              )}
              ...
              {job.benefits && job.benefits.length > 0 && (
                <>
                  <h4 className="job-modal__subheading">Benefits</h4>
                  <ul className="job-modal__bullet-list">
                    {job.benefits.map((benefit, idx) => (
                      <li key={idx}><span className="job-modal__bullet-dot" />{benefit}</li>
                    ))}
                  </ul>
                </>
              )}
              ...
              <p className="job-modal__desc-paragraph">
                <strong>{agencyName}</strong>, based in {agencyLocation}, brings {agencyExperience.charAt(0).toLowerCase() + agencyExperience.slice(1)},
                with {agencySupport.charAt(0).toLowerCase() + agencySupport.slice(1)}. {agencyCertifications}.
              </p>
            </div>
          </div>
          ...
          <div className="job-modal__section">
            ...
            <div className="job-modal__grid">
              ...
              <div className="job-modal__field">
                <span className="job-modal__field-label">Country</span>
                <span className="job-modal__field-value">{job.country || 'United States'}</span>
              </div>
              {job.facility && (
                <div className="job-modal__field">
                  <span className="job-modal__field-label">Client / Facility</span>
                  <span className="job-modal__field-value">{job.facility}</span>
                </div>
              )}
              <div className="job-modal__field">
                <span className="job-modal__field-label">Facility Type</span>
                <span className="job-modal__field-value">{job.facilityType || job.workSetting}</span>
              </div>
            </div>
          </div>
          ...
          <div className="job-modal__section job-modal__section--disclaimer">
            ...
            {job.jobBoardDisclaimer ? (
              <p className="job-modal__overview">{job.jobBoardDisclaimer}</p>
            ) : (
        ...
        <div className="job-modal__footer">
          {readOnly ? (
            <>
              <span className="job-modal__share-label">Preview only — sharing and applications are disabled until published.</span>
              <div className="job-modal__actions">
                <button type="button" className="btn btn--primary btn--sm" onClick={onClose}>Close Preview</button>
              </div>
            </>
          ) : (
            <>
